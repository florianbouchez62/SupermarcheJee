		...
		<div class="row">
			<div class="col s12">	
				<c:if test="${panier.size() > 0}">								
						 	...
						 	<c:set var="totalTVA" value="${0}" />
						  	<c:forEach items="${panier}" var="article">
							    <tr>
							      ...
							      <td><fmt:formatNumber value="${article.value.prixHT / 100}" pattern=".00"/> EUR</td>
							      <td><fmt:formatNumber value="${article.value.prixTTC / 100}" pattern=".00"/> EUR</td>
							      <td><fmt:formatNumber value="${article.value.montantTVA / 100}" pattern=".00"/> EUR</td>
							      <td>
					...
					<h3>Total TTC : <fmt:formatNumber value="${totalTTC / 100 }" pattern = ".00"/> EUR</h3>
					<p>dont TVA : <fmt:formatNumber value="${totalTVA / 100 }" pattern = ".00"/> EUR</p>
					<p>Total HT : <fmt:formatNumber value="${totalHT / 100 }" pattern = ".00"/> EUR</p>
				</c:if>
				<c:if test="${panier.size() == 0}">
